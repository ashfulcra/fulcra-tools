...
## Environment
- `FULCRA_CLI_COMMAND` — override the storage CLI (default `fulcra-api`). E.g. an absolute path to the installed binary (`$(uv tool dir --bin)/fulcra-api`) — never `uv tool run` (resolves ephemerally).
