...
| **Addressing** | The role is resolved to a **holder identity** at send time and the `tell` goes *there*. Since coord-engine v1.6.9 (#410) this is a **detector policy, not an engine requirement**: `briefing`, `inbox`, `needs-me`, and `listen` all fold role-addressed directives into the holder's queue through one bounded resolver (`COORD_ROLE_FOLD_BUDGET`), so a holder running an ordinary heartbeat DOES see role-routed work. We still pre-resolve to the holder because delivery-to-an-identity is checkable at send time (rc + presence), while delivery-to-a-role depends on the recipient's fold running and its lease being readable at that moment — the stricter policy buys a verifiable send, not correctness the engine lacks. |
