...
- **Output**: one-line JSON envelope `{ok, total, posted, skipped_existing, verified, errors:[{stage,message}]}` (stages: `args|parse|auth|post` — exactly what the code emits). Exit 0/2. The agent narrates this conversationally; `skipped_existing` is currently always null (the ingest endpoint gives no dedup feedback), so narration reports posted/total, never duplicate counts.
...
**Note — slim dates are always 20YY** (smoke-test lesson, 2026-07-04/05): `parse_netflix_date` maps two-digit years to `2000 + YY` (fulcra-media parity; real Netflix slim exports cannot predate 2000, so this is correct for user data). A test fixture dated `1/2/71` therefore imports as **2071**, not 1971 — which briefly masqueraded as a platform indexing bug when readback against a 1971 window came up empty. There is no indexing issue; the importer, readback verify, and `DeletedRecord` tombstone flow are all proven live end-to-end (import → readback → tombstone → verified gone, at both present-day and 2071 event times). Methodology: smoke fixtures must use dates the slim format can actually represent, and collision-safety comes from distinctive fake titles (det-ids include the title), not from exotic dates. Pre-2000 rows only exist in the GDPR variant, whose four-digit timestamps parse literally.
