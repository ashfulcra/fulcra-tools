...
- ``kind=response`` for the same slug CLOSES it, whoever sent it: ``respond``
  and ``task done`` both emit one.
    ...
    out_rows = tuple(
        ...
        for slug, meta in sorted(state["open"].items(),
                                 key=lambda kv: (kv[1].get("pri") or "P3",
                                                 kv[1].get("at") or "")))
    note = f"as-of {state['cursor']}; +{fresh} events; state {source}"
    if not persisted:
        note += "; STATE WRITE FAILED — cursor not advanced durably, next pass replays"
