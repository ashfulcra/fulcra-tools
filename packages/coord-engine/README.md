...
| `COORD_RETENTION_CAP` | `20` | count | Maximum task/review archives in one reconcile pass. Positive integers override the unattended safety cap; invalid, zero, or negative values retain 20. |
| `COORD_RETENTION_FORCE` | unset | boolean | Exact value `1` bypasses the once-daily retention marker for that invocation. The pass still writes the marker; other values preserve the normal throttle. |
