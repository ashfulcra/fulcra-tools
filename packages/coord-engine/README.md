...
```bash
uv tool install "git+https://github.com/ashfulcra/fulcra-tools@coord-engine-v1.6.7#subdirectory=packages/coord-engine"
...
| `COORD_ACKS_FULL_EVERY` | `12` | count | Passes between FORCED full ack folds in `reconcile`. The fold is change-driven (it asks the store what changed and re-folds only those slugs); this bounds how long a change the query never reported can persist, and carries the orphan-shard GC, which only rides the full fold. `1` disables the incremental path (every pass lists every ack dir). Any doubt — no change query, a query error, no `generated_at` anchor — full-folds regardless of this knob. |
