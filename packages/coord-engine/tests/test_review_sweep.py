...
def test_unreadable_listing_closes_nothing():
    d, why = review_gc.sweep_disposition(
        None, settled_marker=SETTLED, listing_ok=False)
    assert d == review_gc.SWEEP_UNKNOWN
    assert "closes nothing" in why


def test_non_enumerable_listing_closes_nothing():
    d, _ = review_gc.sweep_disposition(object(), settled_marker=SETTLED)
    assert d == review_gc.SWEEP_UNKNOWN
...
def test_every_terminal_marker_closes_and_is_named(marker):
    d, why = review_gc.sweep_disposition({marker}, settled_marker=SETTLED)
    assert d == review_gc.SWEEP_CLOSE
    assert marker in why, "the closure must name the marker it acted on"
...
def test_a_marker_added_to_the_set_is_honoured_without_editing_the_sweep(
    ...
    d, why = review_gc.sweep_disposition({".future-marker"},
                                         settled_marker=SETTLED)
    assert d == review_gc.SWEEP_CLOSE
    assert ".future-marker" in why


def test_no_markers_at_all_stays_open():
    d, _ = review_gc.sweep_disposition({SHARD}, settled_marker=SETTLED)
    assert d == review_gc.SWEEP_OPEN
...
def test_settled_cache_closes_and_records_that_the_digest_was_recomputed():
    ...
    assert d == review_gc.SWEEP_CLOSE
    assert "recomputed" in why and "matched" in why
...
def test_a_stale_digest_does_not_close_the_row():
    ...
    d, _ = review_gc.sweep_disposition(
        names, settled_marker=SETTLED, marker_fm=fm)
    assert d == review_gc.SWEEP_UNRESOLVED
...
def test_a_missing_evidence_KEY_is_quarantined_not_merely_unresolved():
    ...
    assert d == review_gc.SWEEP_UNKNOWN_PROVENANCE
...
def test_terminal_marker_wins_over_a_settle_marker_in_the_same_listing():
    d, why = review_gc.sweep_disposition(
        ...
        marker_fm={"state": review.APPROVED})
    assert d == review_gc.SWEEP_CLOSE
    assert review_gc.CONCLUDED_MARKER in why
