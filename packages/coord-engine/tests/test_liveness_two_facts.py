...
`work_measured` is the part I got wrong on the first pass and had to correct.
I initially treated "no timestamp" as UNKNOWN and suppressed the nudge — which
...
signal loss: a genuinely dark agent would have stopped being surfaced anywhere.
So three states, not two — nobody measured (legacy behaviour, unchanged),
measured and found work, measured and found none (a real finding; nudge stands).
"""
...
def test_a_stale_beat_with_fresh_work_is_not_nudged():
    ...
    lv = presence.liveness(_shard("2026-08-03T11:49:00Z"), now=NOW,
                           work_ts="2026-08-09T08:30:00Z", work_measured=True)
...
def test_both_facts_are_rendered_never_fused():
    ...
    lv = presence.liveness(_shard("2026-08-03T11:49:00Z"), now=NOW,
                           work_ts="2026-08-09T09:00:00Z", work_measured=True)
    ann = lv["annotation"]
...
def test_the_nudge_survives_when_both_are_stale():
    ...
    lv = presence.liveness(_shard("2026-08-01T00:00:00Z"), now=NOW,
                           work_ts="2026-08-01T06:00:00Z", work_measured=True)
...
def test_measured_and_empty_still_nudges_and_says_which_it_is():
    ...
    lv = presence.liveness(_shard("2026-08-03T11:49:00Z"), now=NOW,
                           work_ts=None, work_measured=True)
    ann = lv["annotation"]
...
def test_a_live_beat_is_unaffected_by_work_evidence():
    ...
    lv = presence.liveness(_shard("2026-08-09T11:55:00Z"), now=NOW,
                           work_ts="2026-08-09T11:00:00Z", work_measured=True)
