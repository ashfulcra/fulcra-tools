...
- **Low-confidence timestamps are labelled in the note, and the wording follows PROVENANCE.** The importers compute `timestamp_confidence` correctly, but the typed ingest path has no slot for that field and drops it on the wire, so `note` is the only channel that reaches a human reading their own timeline. Events carrying `timestamp_cluster_size` (Trakt: >= N rows sharing one `watched_at`) get `[bulk import — time unreliable]`; every other low-confidence event gets the source-agnostic `[time unreliable]`. That distinction matters — Netflix's date-only rows get a synthetic noon ("we know the day, not the time") and Apple TV's Recently Watched rows get a fetch-time upper bound ("some time before this"); calling either a bulk import would state something false, and a reader who catches one wrong label stops trusting every label. Neither marker **corrects** the timestamp: the true time is unknown, and a confidently invented one would be worse than a labelled wrong one. Without any label a bulk event is indistinguishable from real viewing — on 2026-08-14 Trakt sent 128 items, 91 sharing one identical timestamp, which landed as 110 hours of television inside a 2h50m window.
