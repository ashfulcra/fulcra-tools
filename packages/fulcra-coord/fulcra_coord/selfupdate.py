...
def _resolve_update_plan() -> Optional[tuple[list[list[str]], Optional[str]]]:
    """Resolve (argv list, cwd) for the update — from LOCAL config ONLY.

    Precedence: an explicit ``update-cmd.json`` {"cmd": [...], "cwd": ...}
    wins (full operator control); else ``update.json`` {"checkout": ...}
    yields the built-in safe default with both argvs BUILT IN CODE from the
    configured path — the pointer rule's exec-side half: no string from the
    bus is ever part of an argv. No (valid) config -> None (the caller
    degrades visibly). There is deliberately NO default checkout path: the
    package cannot reliably know where the operator's canonical clone lives,
    and guessing wrong would `git pull` someone else's directory."""
    cmd_cfg = _load_json_config("update-cmd.json")
    cmd = cmd_cfg.get("cmd")
    if (isinstance(cmd, list) and cmd
            and all(isinstance(t, str) and t for t in cmd)):
        cwd = cmd_cfg.get("cwd")
        return [list(cmd)], (cwd if isinstance(cwd, str) and cwd else None)
...
def _run_update(plan: tuple[list[list[str]], Optional[str]]) -> bool:
    ...
    session boot and a polling tick)."""
    argvs, cwd = plan
...
def maybe_self_update(*, backend: Optional[list[str]] = None,
    ...
    updated | degraded-no-config | update-failed | error. A successful update
    does NOT re-exec — this process logs 'takes effect next invocation' and
    continues on its already-imported code; the next wake/session runs new."""
    try:
