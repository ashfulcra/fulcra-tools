...
| `start` | Create a new task from a TITLE. A task-id-shaped title (`TASK-YYYYMMDD-…`) is refused with exit 1 — that's almost always a mis-aimed claim; to claim/activate an existing task use `update <id> --status active` |
...
| `FULCRA_COORD_CRITICAL_TIMEOUT_SECONDS` | `max(reconcile+60, 150)` | Reserved budget for reconcile's cheap-but-critical role-health / vacancy-escalation and undelivered-directive checks. At scale the core view refresh alone can spend the whole reconcile budget (~85s of 90s at ~800 tasks), starving those checks — role leases then stop getting their SLA escalation and directives rot in dead inboxes. They run against this larger deadline so they still complete past the core ceiling. Retention stays gated on `FULCRA_COORD_RECONCILE_TIMEOUT_SECONDS`, so a tight budget keeps it skipped |
...
| `FULCRA_COORD_VIEW_SKIP_UNCHANGED` | `1` | Every **write** rebuilds **all** views but uploads **only the ones whose content actually changed** since the last *confirmed* upload — sha256 fingerprint of the exact upload serialization (per-rebuild timestamps excluded), recorded locally per host, **success-only**. The all-views fan-out scales with fleet size (~55 views on a 33-identity bus) while a `tell`/`update`/`done` changes ~5 — under backend 504-weather the ~50 redundant uploads turned every write into a partial failure and reconcile's repair backlog **grew** instead of draining (2026-06-10 evidence). A view whose upload **failed** keeps its stale fingerprint and is re-attempted on the next write even if its content is unchanged (the local view cache is written on failure too, so the cache can never drive this decision); the `generated_at`-stamped freshness beacons (`views/summaries.json`) always upload so `FULCRA_COORD_VIEW_STALE_MIN` never trips on a quiet-but-healthy bus. **Reconcile never skips**: a fingerprint proves only what *this host* last uploaded, not the remote's current content (no compare-and-swap; views are shared mutable paths another host can overwrite), so reconcile re-asserts **every** rebuilt view — cross-host view drift is repaired within one reconcile cadence while the hot write path keeps the fan-out cut. `0` restores upload-everything on the write path |
...
| `FULCRA_COORD_MESSAGE_TTL_DAYS` | `7` | A still-`proposed` **message-class** directive — a `tell`/FYI/verdict echo with a **concrete** assignee and `expects_response` falsy — whose `created_at` is older than this is transitioned `proposed → done` by the reconcile retention pass (evidence: `delivered message auto-closed after N days (message-class TTL)`), after which cold-archive sweeps it out of the hot path on a later pass (recoverable via `fulcra-coord restore`). Delivered messages carry information and expect nothing back, so nobody ever marks them done and they pile up as `proposed` forever, bloating every listing. **Deliberately never auto-closed**: anything with `expects_response` truthy (the **closed-loop guarantee** — an open ask stays open until a bus-native response, period), review/dispatch/question/signoff (and unknown) loop kinds, broadcasts (their own expiry above), `kind:idea` backlog items, self-owned work tasks, non-proposed statuses, and clockless messages (missing/unparseable `created_at` — fail-safe). Reconcile reports `closed N message(s)` in its Retention line |
...
5. Uploads the views whose content actually changed since their last confirmed
   upload (sha256 fingerprint, success-only — see
   `FULCRA_COORD_VIEW_SKIP_UNCHANGED` above); unchanged ones are skipped
6. Writes an operation marker — if view uploads fail, flags `needs_reconcile`
7. `fulcra-coord reconcile` repairs partial writes — and, unlike the write
   path, re-uploads **every** rebuilt view (no fingerprint skip), so a view
   another host overwrote is re-asserted within one reconcile cadence
