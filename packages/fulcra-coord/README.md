...
| `ensure-codex-watch` | Idempotently (re)arm Codex coordination in one shot — installs the Codex hooks, installs the per-agent inbox listener, best-effort `launchctl load`s it (`--no-load` to skip), and optionally refreshes presence (`--no-connect` to skip). Codex SessionStart runs it backgrounded each app start so a missing listener self-heals. Safe to run repeatedly (`--agent`, `--set-identity`, `--can-review`/`--role`, `--interval-min N`, `--dry-run`) |
