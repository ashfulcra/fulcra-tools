...
//
// Golden cross-language parity tests for the event->ingest-record transform.
// The expected values were produced by running the Python daemon transform
// (fulcra_attention.build_attention_event + IngestPipeline.build_record) on
// the same inputs and hardcoded here. If these ever diverge, the relayless
// extension and the daemon would emit different source_ids / payloads.

import { describe, test, expect } from "vitest";
import { buildWireRecord, sourceId, encodeBatch } from "../../src/relayless/wire";
import type { AttentionEvent } from "../../src/types";

const CTX = { definitionId: "def-attn-123", tagIds: ["tag-attn", "tag-web"] };
...
describe("source_id golden vector", () => {
  test("url variant matches Python byte-for-byte", async () => {
    // Python: source_id(key=scrub_url(url), start_time=parse('2026-05-18T14:00:00.500Z'))
    // scrub drops utm_source + fragment -> https://example.com/article?id=42
    const sid = await sourceId(
      "https://example.com/article?id=42",
      "2026-05-18T14:00:00.500Z",
    );
    expect(sid).toBe("com.fulcra.attention.v2.ab5a4bdc52e52410");
  });

  test("category variant matches Python byte-for-byte", async () => {
    const sid = await sourceId("Work", "2026-05-18T14:00:00Z");
    expect(sid).toBe("com.fulcra.attention.v2.b6c319f1205848f9");
  });
...
describe("wire record shape (url variant) matches Python", () => {
  test("full record", async () => {
    ...
    expect(record.metadata.source).toEqual([
      "com.fulcra.attention.v2.ab5a4bdc52e52410",
      "com.fulcradynamics.annotation.def-attn-123",
    ]);
    expect(record.metadata.tags).toEqual(["tag-attn", "tag-web"]);
  });

  test("inner data string is byte-identical to Python json.dumps(sort_keys=True)", async () => {
    const { record } = await buildWireRecord(urlEvent(), CTX);
    const expected =
...
describe("wire record shape (category variant) matches Python", () => {
