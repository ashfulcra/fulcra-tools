...
| **coord tracker bridge** *(alpha)* | Provider-neutral projection primitives for mirroring coord work into an external tracker without making that tracker authoritative. Phase 1 ships normalized snapshots, full source-identity state, versioned policy, and a pure diff planner; source and tracker adapters follow in later gated phases. | [`README.md`](packages/coord-tracker-bridge/README.md) |
