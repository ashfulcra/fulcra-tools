...
| **Hermes "press play" demo** | Operator tooling for the Fulcra press-play demo — spawn per-guest ephemeral [Hermes](https://hermes-agent.nousresearch.com) agents that onboard each person into their own Fulcra account. The active port runs on Vercel Sandbox; the original **Daytona** version was extracted to the standalone (archived) repo [`ashfulcra/fulcra-hermes-daytona`](https://github.com/ashfulcra/fulcra-hermes-daytona). | [`packages/hermes-vercel/README.md`](packages/hermes-vercel/README.md) |
