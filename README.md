...
them on schedule, and serves the onboarding wizard + dashboard. This repo
exists to build and ship Collect.

Everything else in this monorepo is a component or plugin of Collect — the
frontend it serves, the macOS companion, the shared API client, and the data
sources it runs. They were merged in from separate repositories as the project
consolidated (see [History](#history)); they are not co-equal standalone tools.
...
| [`packages/attention`](packages/attention) | Browsing-attention capture: a Collect plugin **plus** a Chrome MV3 extension that POSTs tab/idle events to the daemon. |
