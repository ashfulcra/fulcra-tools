...
| **Linear visibility bridge** *(alpha)* | A `fulcra-workspaces` space projected into Linear so the humans it blocks can see and answer it: one **"Blocked on &lt;me&gt;"** project per consumer, cards assigned to that person so they reach My Issues rather than a project nobody opens, and a return leg that carries their reply back to the member who was waiting. Coordination stays in the workspace; Linear is a messaging surface, so nothing creates work there for agents to pick up. Works on the bare convention or on a coord-engine fold. | [`SKILL.md`](skills/fulcra-agent-linear/SKILL.md) |
