...
- **A CHANGES is lifted only by a shard that NAMES it (`supersedes:`), never by the clock
  ...
  dominating until you re-file). A hand-written APPROVE must carry `supersedes:` itself. **The link is CAUSAL, not nominal (r7, codex-coder):** a name is predictable (selectable
  timestamp, digest of reviewer|verdict|note), so an older APPROVE could predeclare a future CHANGES's
  name and erase it. Every verb-written shard carries a random `nonce:`; a superseder quotes the target
  as `name@nonce` (the verb reads each prior it lists); the fold honours a bare name only for a
  nonce-less legacy target whose STORE mtime (server-assigned) proves it strictly earlier — same minute
  or unknown fails closed. **Invalid edges:** a shard can never resolve
