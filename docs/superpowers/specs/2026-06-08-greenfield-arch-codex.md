...
Build one small append-friendly coordination substrate over Fulcra Files, then keep coordination state and operator visibility as first-class typed projections over the same records rather than as separate feature piles that each rediscover storage, identity, and concurrency rules.
...
- `ActivityEvent`: a normalized, append-friendly record of what happened in the work system: task transitions, progress updates, blockers, review decisions, checkpoint writes, digest emissions, and health changes. It is derived from source-record events where possible and may also be written directly by adapters for lifecycle moments.
...
Operator visibility is a named product goal, not an adapter afterthought. It should live in an `operator_visibility` projection service inside `fulcra-coord-core`, fed only by `Task`, `Directive`, `Presence`, `ActivityEvent`, `ReviewDecision`, `ContinuityCheckpoint`, and `Health` records. Its outputs are views, not new workflow authority:

- `views/operator/timeline.json`: chronological activity stream across agents, workstreams, blockers, review decisions, and checkpoints.
- `views/operator/agents.json`: what each agent is doing now, when it was last seen, current work, blocked work, and recent history.
- `views/operator/blockers.json`: human blockers, external blockers, stale active work, failed reviews, and degraded health.
- `views/operator/digest-input.json`: the structured input for morning/evening digest generation.
- Optional annotation writers mirror this data to Fulcra timeline tracks, but annotations consume visibility views; they do not become the source of truth.

This keeps visibility first-class without re-tangling the coordination core: task lifecycle emits typed events; visibility reducers read them; adapters render or push them. The core remains authoritative for work state, while the operator surface remains authoritative for "what is happening?"
...
Sixth, promote operator visibility from accumulated side effect to product surface. The current annotations, digest, health, presence, and resume features are not mistakes; they are evidence of a real operator need. The departure is to make them one visibility layer fed by core records instead of many subsystems each reaching back into lifecycle code.

Seventh, treat adapters as adapters. Claude Code, Codex, OpenClaw, ChatGPT facade, launchd, cron, and hooks should all use the same core services. No adapter should own product rules such as "reviewer cannot merge unreviewed Codex work" or "do not checkpoint on every update."
...
Keep the operator surfaces. `resume`, `needs-me`, per-agent activity, timeline/history, twice-daily digest, stale-task health, and direct inbox notification are the product center. They should become smaller, cleaner projections over better records.
...
Set a package boundary rule: `fulcra-coord-core` is allowed to coordinate work, route directives, surface operator state, and link checkpoints. Operator visibility is explicitly in scope: timelines, per-agent now/history, progress, blockers, review state, digest input, and health are part of the product. It is not allowed to become a generic agent runtime, a GitHub client, a memory system, a notification platform, or a chat facade. Those belong in adapters or separate packages.

Define a visibility contract for every mutation: if a command changes what an agent is doing, what is blocked, what needs review, what can be resumed, or what the human should know, it must emit or update the typed record that `operator_visibility` consumes. This keeps visibility complete without letting digest/annotations/health logic call back into task mutation internals.
...
Keep forge and vendor integrations behind artifact adapters. GitHub, Claude Code, Codex, OpenClaw, ChatGPT, launchd, cron, Slack, Discord, and ntfy should never be imported by the core reducers. They produce or consume bus records or visibility views.
