...
> Do not implement it task-by-task. Some paths this plan names exist in the
> shipped tree and some never appeared; verify each against the tree before
> acting on any of them. Read it for the reasoning; the shipped tree is the
> authority. Current coordination doctrine lives in
> [`AGENTS.md`](../../../AGENTS.md).
