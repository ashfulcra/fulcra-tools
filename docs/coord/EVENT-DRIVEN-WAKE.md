...
| Claude Code web/cloud UI | No exact-session inbound wake is documented. Do not substitute a different Managed Agents session without explicit migration. | Router-owned alignment marker for the agent's self-armed platform Routine; `no_session_created: true` |
