...
measurement is required or licensed, and the release does not set
`public_read_epsilon_verified` or treat a current generation as public
...
verification from two credentialed hosts across `queue`, `needs-me`, review,
forge, roles, presence, and reconcile. A required `UNKNOWN`, degraded fallback,
or nonzero result blocks the adoption claim. Release, adoption,
generation-backed serving, and cursor-v2 activation are independent facts.
