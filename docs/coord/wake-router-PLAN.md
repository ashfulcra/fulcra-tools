...
| W3 | **Zero-token TTL sweep.** `coord-engine engagement sweep <team>` (host tick, model-free): session agents past TTL get continuity-parked + roles released per the idle-reaping rule; writes a visible PARKED marker; idempotent; never fires on `resident`/`occasional`. | W2 | coord-maintainer |
...
| W5.5 | **Thin host executor.** A policy-free, model-free poller on each resident host with host-local adapters: reads `_coord/router/queue/` entries whose `executor` matches its own id, executes via the local adapter script, records the outcome (delivered ⇒ idempotency-keyed delivery-record shard; exhausted ⇒ dead-letter transition, which it owns as claim-holder). It is the SOLE executor for host-local adapters; it executes only what the decision plane resolved (`executor` field), claims by id-match + `claimed_at`, and is idempotent under retry. No policy, no config authority, ~small enough to read in one sitting; a dead executor leaves wakes visibly queued. | W4, W5 | coord-maintainer |
...
| W9 | **FulcraBot PAT custody.** Fine-grained per-repo PAT (Ash's one creation click), router/env-config custody, rotation cadence documented, attribution conventions in AGENTS.md. | — (parallel, small) | coord-boss + Ash |
...
2. Session TTL default: `join + 8h` remains PROPOSED, not decided. W1 therefore ships with
   **`--engagement session` without `--until` REJECTED** (explicit error naming the pending
   operator decision) — no default is silently encoded. When Ash confirms a default, a follow-up
   one-liner adds it; until then omission is an error, so no task behavior is left open.
