...
> **Historical implementation plan.** This document preserves the task breakdown
> and acceptance criteria. It is not a record of a particular team's deployment.

**Parent spec:** [`wake-router-SPEC.md`](wake-router-SPEC.md).
**Gate:** independent reviewer plus component owner before dispatch and at each task.
...
- **Cursor + durable dedup** (`_coord/router/cursor.json`): `{watermark: <iso8601Z>,
  processed: {<idempotency-key>: <first-seen iso8601Z>, …}}`. Under shipped E3, the watermark is
  the second-granular `data-updates.uploaded_at` high-water mark (monotonic: never written
  backwards); `processed` is the durable
  ...
  equal feed timestamps are possible, so a strict `uploaded_at > watermark` scan could skip an
  equal-timestamp shard that landed after checkpoint. Each scan therefore rescans **inclusively**
  (`uploaded_at >= watermark`) and relies on the processed ledger to suppress already-handled
  keys. The original W4 minute-granular listing scan remains the fail-closed fallback on feed
  doubt, preserving its inclusive-mtime behavior. Checkpoint write is atomic (write-temp +
...
**The decision plane is cloud-hosted; resident hardware is never mandatory.** the operator's rule,
...
- **Decision plane (W4)** runs in a cloud session environment — designated: the
  `cloud coordinator` environment (already carrying the setup-script + Routine + env-config
  stack). **The parent spec's hosting sentence and namespace-writer model are updated in this same PR**
  (operator decision, the operator 2026-07-22) so exactly one authoritative contract exists — spec and plan
...
1. The decision plane is cloud-hosted; host-local adapters use the W5.5 executor.
2. Session TTL defaults to `join + 8h` when `--engagement session` omits `--until`.
   Expiry means LAPSED with reduced-cadence check-ins; parking is explicit.
