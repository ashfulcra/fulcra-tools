...
| skills/fulcra-agent-* (11 coord2 skills) | 7/11 have ordered probe tables: automation (installed-jobs probes), presence/roles/tasks, and now continuity/review/directives (added by this PR — every probe verb pinned to cli.py by tests/test_skill_probes.py); 4 remain (health has `doctor` already but no probe table, forge, operator, reconcile) | engine folds fully unit-tested; the automation skill's bash installers have no CI tests | biggest win remaining: bats-or-subprocess tests for install-listener.sh / install-heartbeat.sh |
| packages/csv-importer/skills/fulcra-csv | YES (added by this PR) — ordered authed→def→dry-run→landed probe table | YES (covered by the package's `tests/` suite: parser/export/confidence) + contract test (tests/test_skill_contract.py) added by this PR | flag drift clean in both directions (every documented flag is a real click.option and vice-versa); source-id-prefix default pinned |
| packages/media-helpers/skills/fulcra-media | YES (added by this PR) — ordered authed→bootstrapped→check-only→watermark probe table, consolidating the pre-existing `--check-only`/heartbeat probe concepts | YES (covered by the package's large `tests/` suite: importers, CLI, dedup, watermarks) + contract test (tests/test_skill_contract.py) added by this PR | contract test pins importer roster names, state.json keys, and an exact set of import-path envelope stages (webhook's bind/ready/shutdown excluded); flags intentionally delegated to `--help` are held in an explicit UNDOCUMENTED_OK allowlist (mostly per-importer flags, plus 6 that belong to the non-importer `webhook`/`reset` commands) so a new CLI flag forces a document-or-allowlist decision |
