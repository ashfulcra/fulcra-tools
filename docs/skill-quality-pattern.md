...
| skills/fulcra-agent-* (13 coord skills) | ALL 13 covered: 12 have ordered probe tables (automation, presence/roles/tasks, continuity/review/directives, reconcile/forge/operator, atc — every probe verb pinned to cli.py by tests/test_skill_probes.py — plus durable-state, whose probes run on `fulcra-api` directly and are pinned by the same test file) + health, which is itself the `doctor`/fleet-fold probe surface (no table of its own needed) | engine folds fully unit-tested; automation installers now have CI tests (install-listener.sh / install-heartbeat.sh, PATH-shim harness) | coord adoption complete |
