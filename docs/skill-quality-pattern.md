...
| skills/fulcra-agent-* (11 coord2 skills) | automation has one (installed-jobs probes); presence/roles/tasks now have ordered probe tables (added by this PR — probe verbs pinned to cli.py by tests/test_skill_probes.py); 7 skills remain (continuity, directives, forge, health, operator, reconcile, review) | engine folds fully unit-tested; the automation skill's bash installers have no CI tests | biggest win remaining: bats-or-subprocess tests for install-listener.sh / install-heartbeat.sh |
