...
set -u
PIN="d187e546c1dfcccfc240ccc4fabebdb725d92643"   # coord-engine at d187e546. NINETEENTH pin move. Carries PR 755: a DIRECTED v4 close is now mirrored to `for_agent` (the ASSIGNEE, whose channel holds the open) instead of `to` (the OWNER being notified). Before this, closing a directive filed its v4 close in the asker's channel, so the assignee's fold owed the row forever and every fold-served count OVER-reported. Measured 2026-09-08: collect-maintainer closed ten review-request rows for PR 753/754 at 09:55:48Z; codex-reviewer's checkpoint at cursor 10:19:17Z with unread_events 0 still held all ten open - the fold had read past the closes and never saw them. The broadcast half of this was fixed under ruling 55b1056b; the directed half had the identical flaw and was missed. Rows stranded BEFORE this build still need a coord-fold close against their doc; adopting only stops new ones accruing. Adopting flips nothing (the switch is already fold). No re-seed. Engine delta from 720ce522 is one line in records.py plus tests. Everything in 720ce522 is still here.
