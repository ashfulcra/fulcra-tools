...
set -u
PIN="e06e69e5d44d92b2b52a09020f53f2bd1ccdc1d5"   # coord-engine at e06e69e5, 32 commits past 985a4be3; the build self-reports 2.0.6 because __version__ did not move with it - identity is the BUILD COMMIT, as the fast path below already insists. What the fleet gains: (1) a 500 from the store no longer reads as an absent file - it was five sites and every one of them WRITES, so the old engine could overwrite a document it had merely failed to fetch; (2) obligations folds from the stream against a durable checkpoint, so a pass costs the NEW events rather than the corpus, and seeding names its own gaps instead of refusing on unrelated ones; (3) roles publish, and a roles claim distinguishes the three states - an unreadable read is UNKNOWN, an unverified write is not a claim - with --repair-unknown to break the carried-UNKNOWN deadlock; (4) review verdicts get a real supersession edge: same-second shards order by chronology, the winning shard is exposed, and a CHANGES is lifted only by a shard that NAMES it, quoting a causal nonce and binding the target's content digest against the store's mtime - never by the clock and never by a predictable name, because a name the constrained party can guess is forgeable; a shard cannot resolve itself and an invalid edge is surfaced, not folded around; (5) escalate stops restating a ROLE VACANT already on the board (the daily re-mint); (6) intent emits its companion, blocked is read as a bus signal and not only a field, a prefix seals only when its section actually loaded, and a documented-optional courtesy file no longer blocks all publication. Forge section refusal and the cold-start build budget remain separate open P1s.
