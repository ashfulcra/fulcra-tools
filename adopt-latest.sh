...
set -u
PIN="aa2dc4c8a8e2cd0cb0a5113b5f873a26d5f03cfe"   # coord-engine at aa2dc4c8. SEVENTEENTH pin move. Carries PR 749: under serve=fold the obligation components now PARTITION the folded rows, so each open is counted once. Before this, `tasks` carried every row and `directives`/`reviews` carried the same rows again, and obligations.fold extends one owed list per component, so every fold-served count was DOUBLE (measured 2026-09-07 by coord-opus-worker: open=3 answered DATA 6; coord-boss 606 vs compare n=303; codex-coder 142 vs 71; arc 40 vs 20). States, ordering and rows were always right; only the totals were inflated. Adopting flips nothing (the switch is already fold). No re-seed. Engine delta from 1994a3ea is cli._fold_probes + tests. Everything in 1994a3ea is still here.
