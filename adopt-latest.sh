...
set -u
PIN="985a4be36a29e3e0fcc3065e729d96b3a4604c8d"   # coord-engine 2.0.6: 685 redelivery never resurrects an ANSWERED obligation. 2.0.5 made engine-minted opens emit, which created the backward case: a close made during the emit-less window had no open to answer, so redelivery replayed the open into a fold that had never seen a close - terminal in the document, OPEN in the stream, permanently. Refuses ANY terminal state because abandoned->done is an illegal transition, so a resurrected abandoned row is undischargeable by its holder. UNKNOWN outranks terminal: an existing-but-unreadable candidate fails closed rather than being outvoted by an answered sibling. Forge section refusal and the cold-start build budget remain separate open P1s.
