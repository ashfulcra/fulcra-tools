...
set -u
PIN="720ce522eeca4ceb79af13c59bbcdafd958795cd"   # coord-engine at 720ce522. EIGHTEENTH pin move. Carries PR 751: a review-request directive now writes the SAME priority (P1) to its task document that it has always emitted on its companion event. Before this the document took tasks' default P2, so the row carried two priorities and compare-to-fold — which compares (slug, pri, ptr) — reported that slug in BOTH only_old and only_new on every pass, forever, on a row nothing was wrong with (measured 2026-09-07 on codex-reviewer: four review-request slugs, identical pointers, doc P2 against fold P1). Rows already written keep their split until they close. Adopting flips nothing (the switch is already fold). No re-seed. Engine delta from aa2dc4c8 is one priority argument plus its test. Everything in aa2dc4c8 is still here.
